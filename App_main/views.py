...
def user_dashboard(request):
    ongoing_bookings = BookingModel.objects.filter(user=request.user, status='Service Confirmed')
    complete_service = BookingModel.objects.filter(user=request.user, status='Service Provided')
    content = {
        'ongoing_bookings': ongoing_bookings,
        'complete_service': complete_service,
        'loginForm': loginForm_(),
    }
    return render(request, 'App_main/dashboard.html', context=content)
...
def invoice_view(request):
    completed = BookingModel.objects.filter(user=request.user, status="Service Provided")
    content = {
        'completed': completed,
        'loginForm': loginForm_(),
    }
    return render(request, 'App_main/invoice.html', context=content)


def single_invoice_view(request, pk):
    book = BookingModel.objects.get(id=pk)
    invoice_row = InvoiceModel.objects.filter(user=request.user, booking=book)
    ...
    return render(request, 'App_main/single_invoice.html', context=content)


def service_man_dashboard(request):
    return render(request, 'App_main/mechanic_dashboard.html')


def service_man_customer_view(request):
    content = {
    ...
    if request.method == 'POST':
        ...
        bookings = BookingModel.objects.filter(user=user, status='Service Confirmed')
        content['services'] = bookings
        content['user'] = user
        return render(request, 'App_main/mechanic_all_services_showcase.html', context=content)
